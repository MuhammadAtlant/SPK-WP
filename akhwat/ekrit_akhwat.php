...
$sql = "SELECT * FROM kriteria_akhwat WHERE id_krit_akh = '$id_krit_akh'";
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Query untuk memperbarui data kriteria akhwat
        $update_sql = "UPDATE kriteria_akhwat
        SET nama_kriteria = '$nama_kriteria', 
        t_kepentingan = '$t_kepentingan',
        cost_benefit = '$cost_benefit' WHERE id_krit_akh = '$id_krit_akh'";
        if (mysqli_query($connection, $update_sql)) {
            echo '<script>alert("Data Kriteria Murid Akhwat berhasil diupdate!");</script>';
            echo '<script>window.location.href="krit_akhwat.php";</script>'; 
        } else {
...
<div class="form-group">
  <label for="id_krit_akhwat">ID Kriteria</label>
  <input type="text" class="form-control" id="id_krit_akh" name="id_krit_akh"  
  value="<?php echo $row['id_krit_akh']; ?>" readonly>
</div>
