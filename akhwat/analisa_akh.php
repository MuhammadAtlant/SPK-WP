  ...
  <!-- Begin menu per murid --> 
    ...
    <h4 class="marquee-text">Selamat Datang  <?php echo $username; ?> </h4>
                ...
                <div class="card shadow mb-6">
                    ...
                    <div class="card-header py-3" style="background-color:thistle">
                <h6 class="m-0 font-weight-bold text-center" style="color:black;">Hasil Analisa Murid Akhwat</h6>
                                </div>
                                <div class="card-body">
                          <center>
				<?php
					...
					// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> vektor S <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< //
					echo "<table class='table table-striped table-bordered text-bold text-primary'>";
					echo "<thead style='background-color:thistle'><tr><th>Alternatif</th><th>Hasil Nilai Perangkingan (V)</th></tr></thead>";
					$total = 0;
					for($i=0;$i<$a;$i++){
						$total = $total + $ss[$i];
					}
					for($i=0;$i<$a;$i++){
						echo "<tr style='background-color:thistle'><td style='color:black'><b>".$alt_name[$i]."</b></td>";
						$v[$i] = round($ss[$i]/$total,6);
						echo "<td style='color:black'><b>".$v[$i]."</b></td></tr>";
					}
					echo "</table><hr>";
					// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> vektor S <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< //
					uasort($v,'cmp');
								for($i=0;$i<$arl2;$i++){ //new for 8 lines below
									...
									elseif($i==($arl2-1)) 
										...
										function jml_kriteria(){
											...
											$kriteria = $connection->query("select * from kriteria_akhwat");
											return $kriteria->num_rows;
										}

										function jml_alternatif(){
                                            include '../backend/koneksi.php';
											$alternatif = $connection->query("select * from alternatif_akhwat");
											return $alternatif->num_rows;
										}

										function get_kepentingan(){
											include '../backend/koneksi.php';
											$kepentingan = $connection->query("select * from kriteria_akhwat");
											if(!$kepentingan){
										...
										function get_costbenefit(){
											include '../backend/koneksi.php';
											$costbenefit = $connection->query("select * from kriteria_akhwat");
											if(!$costbenefit){
												echo $connection->connect_errno." - ".$connection->connect_error;
												exit();
											}
