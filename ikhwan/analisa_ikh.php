  ...
  <!-- Begin menu per murid --> 
    ...
    <h4 class="marquee-text">Selamat Datang  <?php echo $username; ?> </h4>
                ...
                <div class="card shadow mb-6">
                    ...
                    <div class="card-header py-3" style="background-color:steelblue">
                <h6 class="m-0 font-weight-bold text-center" style="color:lemonchiffon;">Hasil Analisa Murid Ikhwan</h6>
                                </div>
                                <div class="card-body">
                          <center>
				<?php
					...
					echo "<thead style='background-color:lemonchiffon'><tr><th>Alternatif</th><th>Hasil Nilai Perangkingan (V)</th></tr></thead>";
					$total = 0;
					for($i=0;$i<$a;$i++){
						$total = $total + $ss[$i];
					}
					for($i=0;$i<$a;$i++){
						echo "<tr style='background-color:lemonchiffon'><td style='color:deepskyblue'><b>".$alt_name[$i]."</b></td>";
								...
								for($i=0;$i<$arl2;$i++){ //new for 8 lines below
									...
									elseif($i==($arl2-1)) 
										...
										function jml_kriteria(){
											...
											$kriteria = $connection->query("select * from kriteria_ikhwan");
											return $kriteria->num_rows;
										}

										function jml_alternatif(){
                                            include '../backend/koneksi.php';
											$alternatif = $connection->query("select * from alternatif_ikhwan");
											return $alternatif->num_rows;
										}

										function get_kepentingan(){
											include '../backend/koneksi.php';
											$kepentingan = $connection->query("select * from kriteria_ikhwan");
											if(!$kepentingan){
										...
										function get_costbenefit(){
											include '../backend/koneksi.php';
											$costbenefit = $connection->query("select * from kriteria_ikhwan");
											if(!$costbenefit){
												echo $connection->connect_errno." - ".$connection->connect_error;
												exit();
											}
