 ...
 <!-- The end of menu per kelas -->
  ...
  <!-- Begin menu per murid --> 
    ...
    <h4 class="marquee-text">Selamat Datang  <?php echo $username; ?> </h4>
                ...
                <h6 class="m-0 font-weight-bold" style="color:sandybrown">Hasil Analisa Kelas An Nasa'i</h6>
                                </div>
                                <div class="card-body">
                          <center>
				<?php
								...
								for($i=0;$i<$arl2;$i++){ //new for 8 lines below
									...
									elseif($i==($arl2-1)) 
										...
										function jml_kriteria(){
											...
											$kriteria = $connection->query("select * from kriteria_perkelas where nama_kelas ='4 An Nasai'");
											return $kriteria->num_rows;
										}

										function jml_alternatif(){
                                            include '../backend/koneksi.php';
											$alternatif = $connection->query("select * from alter_perkelas where nama_kls ='4 An Nasai'");
											return $alternatif->num_rows;
										}

										function get_kepentingan(){
											include '../backend/koneksi.php';
											$kepentingan = $connection->query("select * from kriteria_perkelas where nama_kelas ='4 An Nasai'");
											if(!$kepentingan){
										...
										function get_costbenefit(){
											include '../backend/koneksi.php';
											$costbenefit = $connection->query("select * from kriteria_perkelas where nama_kelas ='4 An Nasai'");
											if(!$costbenefit){
												echo $connection->connect_errno." - ".$connection->connect_error;
												exit();
											}
